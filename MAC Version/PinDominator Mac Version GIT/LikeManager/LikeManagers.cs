...
namespace LikeManager
{
	...
	public delegate void LikeAccount(int AccCount);
    public class LikeManagers
    {
        public static AccortReport_Like objLikeDelegate;
		public static LikeAccount objDelegateNoOfActiveAcc_Like;
		public static LikeAccount objDelegateNoOfDeadAcc_Like;
            ...
            {                    
                if (!isStopLike)
                {
                    ...
                    try
                    {
                        ...
                        {
							GlobusLogHelper.log.Info("[ " + DateTime.Now + " ] => [ Logging With " + objPinUser.Username + " ]");
                            ...
                            if (!checkLogin)
                            {
                                ...
                                if (!checkLogin)
                                {
									GlobusLogHelper.log.Info("[ " + DateTime.Now + " ] => [ Not Logged In With " + objPinUser.Username + " ]");
									DeadAccCount++;
									objDelegateNoOfDeadAcc_Like(DeadAccCount);
                                    return;
                                }
                            }
                        ...
                        if(objPinUser.isloggedin == true)
                        {
                            try
                            {
								...
								ActiveAccCount++;
								objDelegateNoOfActiveAcc_Like(ActiveAccCount);
