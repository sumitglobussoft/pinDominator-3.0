...
namespace PinsManager
{
    ...
    public class ScrapePinManagers
    {
        ...
        public List<string> GetBoardPinsNew_PinScraper()
        {
            try
            {
				GlobusLogHelper.log.Info("[ " + DateTime.Now + "] => [ Scraping Process Started ]");
                                    ...
                                    foreach (var itemArrPin in arrPin)
                                    {
                                        ...
                                        if (!string.IsNullOrEmpty(Pin) && !lstPopularPins.Contains(Pin))
                                        {
                                            ...
                                            if (MaxNoOfPinScrape >= Counter)                                         
                                            {
												...
												GlobusLogHelper.log.Info("[ " + DateTime.Now + "] => [ Pin " + Pin + " ]");
...
//                                                    objPinScraperDelegate();
				...
				GlobusLogHelper.log.Info("[ " + DateTime.Now + "] => [ Total  Pins : " + lstPopularPins.Count + " ]");

				GlobusLogHelper.log.Info("[ " + DateTime.Now + "] => [ Scraping Process Completed ]");
            }
            catch (Exception ex)
            {
                GlobusLogHelper.log.Error(" Error :" + ex.StackTrace);
        ...
        public void ScrapeImage(string Pin)
        {
            ...
            {              
                ...
                try
                {
					GlobusLogHelper.log.Info("[ " + DateTime.Now + "] => [ Scraping Image for Pin " + Pin + " ]");
                        ...
                        try
                        {
                            foreach (string imagepage in ImagePage)
                            {
                                if (!imagepage.Contains("<!DOCTYPE html>"))
                                {
									...
									GlobusLogHelper.log.Info("[ " + DateTime.Now + "] => [ Scraped Image " + ImageUrl + " for Pin " + Pin + " ]");
                                    break;
                                }
