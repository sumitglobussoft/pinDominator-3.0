...
namespace PinsManager
{
	...
	public delegate void AddNewPinAccount(int AccCount);
    public class AddNewPinManager
    {
        public static AccountReport_AddNewPin objAddNewPinDelegate;
		public static AddNewPinAccount objDelegateNoOfActiveAcc_AddNewPin;
		public static AddNewPinAccount objDelegateNoOfDeadAcc_AddNewPin;
                    ...
                    try
                    {
                        ...
                        if (!objPinUser.isloggedin)
                        {
                            ...
                            {                                
                                ...
                                if (!checkLogin)
                                {
									...
									DeadAccCount++;
									objDelegateNoOfDeadAcc_AddNewPin(DeadAccCount);
                                    return;
                                }
                        ...
                        if (objPinUser.isloggedin == true)
                        {
                            try
                            {
								...
								ActiveAccCount++;
								objDelegateNoOfActiveAcc_AddNewPin(ActiveAccCount);
                    ...
                    foreach (Pins pin in UserPins)
                    {
                        try
                        {
                            if (!string.IsNullOrEmpty(pin.Board))
                            {
                                ...
                                {
									GlobusLogHelper.log.Info("[ " + DateTime.Now + " ] => [ " + Board + " Not Found. Creating Board ]");
                                    try
                                    {
                                        BoardNumber = CreateBoard_NewPin(Board, ref objPinUser);
                                    }
                                    catch (Exception ex)
                                    {
                                        GlobusLogHelper.log.Error(" Error :" + ex.StackTrace);
                            ...
                            for (int i = 0; i < ImagePinId.Count(); i++)
                            {
                                ...
                                if (i == 0)
                                {
                                    ...
                                    if (Data.Equals("true"))
                                    {
										...
										GlobusLogHelper.log.Info("[ " + DateTime.Now + " ] => [ Pin Added To " + Board + " From " + objPinUser.Username + " ]");
                                        MaxCount++;
                                        try
                                        {
                                    ...
                                    {
										GlobusLogHelper.log.Info("[ " + DateTime.Now + " ] => [ Pin Not Added To " + Board + " From " + objPinUser.Username + " ]");
                                    }
                                }
                                #endregion
        ...
        public void AddPinFromDevice(ref PinInterestUser objPinUser, ref string BoardId, string Board)
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(BoardId))
                {
                    ...
                    try
                    {
                        BoardId = CreateBoard_NewPin(Board, ref objPinUser);
                    }
                    catch (Exception ex)
                    {
                        GlobusLogHelper.log.Error(" Error :" + ex.StackTrace);
                ...
                if (!string.IsNullOrEmpty(BoardId))
                {
                    ...
                    {
						GlobusLogHelper.log.Info("[ " + DateTime.Now + " ] => [ No Image files selected. ]");
                    }
                }
                else
                {
					GlobusLogHelper.log.Info("[ " + DateTime.Now + " ] => [ Board " + Board + " not created. ]");
                }
            }
        ...
        public string CreateBoard_NewPin(string BoardName, ref PinInterestUser objPinUser)
        {
            try
            {
        ...
        public string GetBoardId_Board(string BoardName, ref PinInterestUser objPinUser)
        {
            try
            {
				...
				GlobusLogHelper.log.Info("[ " + DateTime.Now + " ] => [ Start Getting BoardId For Board " + BoardName + " ]");
				...
				GlobusLogHelper.log.Info("[ " + DateTime.Now + " ] => [ Board Find " + BoardId + " ]");

                return BoardId;
            }
            catch (Exception ex)
            {
				GlobusLogHelper.log.Info("[ " + DateTime.Now + " ] => [ Boards Getting Process Failed ]");
                return null;
            }
        }
        ...
        public void GetBoards(ref PinInterestUser objPinUser)
        {
            try
            {
				GlobusLogHelper.log.Info("[ " + DateTime.Now + " ] => [ Start Getting Boards For User " + objPinUser.Name + " ]");
        ...
        public string NewPin(string Board, string Message, string BlogUrl, ref PinInterestUser objPinUser)
        {
            ...
            {          
                ...
                if (getPinPageSource.Contains("board_id"))
                {
					...
					GlobusLogHelper.log.Info("[ " + DateTime.Now + " ] => [ Repining " + boardId + " For " + objPinUser.Username + " ]");
                }
                else
                {
                    boardId = Board;
