    ...
    public class RepinByKeywordManager
    {
        ...
        public void RepinToBoard(string BoardName, string KeywordToRepin, ref PinInterestUser objPinInUser)
        {
            try
            {
                ...
                {
					GlobusLogHelper.log.Info("[ " + DateTime.Now + " ] => [ Board is not present in " + objPinInUser.Username + "]");                 
                }
                ...
                if (!BoardId.Contains("failure"))
                {
                    if (!string.IsNullOrEmpty(BoardId))
                    {
                        ...
                        {
							GlobusLogHelper.log.Info("[ " + DateTime.Now + " ] => [ Adding Pins From Board :" + KeywordToRepin + " ]");
