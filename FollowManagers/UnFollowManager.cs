    ...
    public class UnFollowManager
    {
        ...
        public bool UnFollow_New(string UserUrl, ref PinInterestUser objPinUserManager)
        {
            try
            {
                ...
                if (objPinUserManager.isloggedin == true)//(ObjAccountManager.LoginPinterestAccount(ref objPinUserManager))
                {
                ...
                {
                    GlobusLogHelper.log.Info(" => [ Login Issue with " + objPinUserManager.Username + " ]");
                    return false;
                }
